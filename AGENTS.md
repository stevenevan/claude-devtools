...
Tauri 2 desktop app for inspecting Codex session execution. Rust reads local JSONL logs from `~/.Codex/`; React renders the recovered execution trace.
...
cd src-tauri && cargo run --bin Codex-devtools-cli -- show-session <projectId> <sessionId> --format json
```
...
All of these read `~/.Codex` only. The one exception is File History's `Save as…` and
...
path to `[Codex root, app-data]`, so it refuses real restore targets, and no pre-write copy was
