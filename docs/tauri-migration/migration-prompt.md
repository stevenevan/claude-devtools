...
- **The frontend seam is already clean.** Only ~16 Wails-binding imports, ~3 `@wailsio/runtime`
  imports, and ~10 `Events.On` calls exist — all concentrated in six adapters under
  `frontend/src/renderer/api/domain/` (`analytics.ts`, `config.ts`, `files.ts`, `maintenance.ts`,
  `sessions.ts`, `system.ts`). Everything else in the frontend calls the stable `ElectronAPI`
  interface (`@shared/types/api`). Swapping adapters is the whole frontend cost.
- **`tauriClient.ts` already exists** (`frontend/src/renderer/api/tauriClient.ts`) — a leftover
  from the original Tauri build. The interface contract is the migration invariant.
...
2. **Interface stays frozen.** `ElectronAPI` / `@shared/types/api` do not change shape. Rust
...
4. **Dual-mode until cutover.** The frontend picks backend (Wails or Tauri) behind a build/env
   flag so every week is shippable and A/B-testable against parity. The flag dies in W15.
...
| **W2** | IPC + event seam | dual-mode client in `frontend/src/renderer/api/` (backend chosen by flag); event bridge Wails `Events.On` → Tauri `listen`; freeze `ElectronAPI` as the parity contract | Frontend runs against Wails unchanged with the abstraction inserted; zero component diffs |
...
| **W16** | Retire Wails + rebuild toolchain | delete Go backend, Wails deps, `@wailsio/runtime`, generated bindings; port build/packaging (macOS `.app` bundle + adhoc codesign, `Taskfile`, `bin/`); rewrite `CLAUDE.md` data-pipeline section; add `docs/tauri-migration/` (inverse of the original migration doc) | Clean Tauri-only build; `cargo test` + `bun test` + `tsc` green; no Go/Wails references remain |
