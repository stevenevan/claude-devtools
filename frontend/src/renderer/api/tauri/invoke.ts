...
// `reviveDates` is OPT-IN per call, default off: the Wails adapters revive only a
// hand-picked subset of results and deliberately skip the rest, so a blanket
// revive would break parity (a raw ISO string on the Wails path would become a
// Date on the Tauri path) and needlessly deep-clone every large payload. Each
// ported method opts in iff its Wails twin does.
