...
import type { CatalogPlugin, MarketplaceView } from '@shared/types/api';
...
// (mirrors PluginsGrid.tsx's card markup). This panel writes nothing.
export const MarketplaceBrowser = (): JSX.Element => {
  ...
  const load = async (): Promise<void> => {
    ...
    setError(null);
    try {
      const catalog = await api.readMarketplaceCatalog();
    ...
    <div className="flex h-full flex-col overflow-hidden">
      <div className="border-border/50 flex shrink-0 items-start justify-between gap-2 border-b px-4 py-3">
        <div>
          ...
          <p className="text-muted-foreground mt-0.5 text-xs">
            Read-only view of plugin marketplaces under ~/.claude/plugins. Nothing here writes.
          </p>
        </div>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="min-w-0 flex-1 overflow-y-auto p-4">
          <MarketplaceDetail marketplace={selected} loading={loading} />
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="grid grid-cols-2 gap-3 lg:grid-cols-3 xl:grid-cols-4">
      {marketplace.plugins.map((plugin) => (
        <PluginCatalogCard key={plugin.name} plugin={plugin} />
      ))}
    </div>
  );
};

const PluginCatalogCard = ({ plugin }: Readonly<{ plugin: CatalogPlugin }>): JSX.Element => (
