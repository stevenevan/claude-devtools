...
- `types/` - Shared type definitions
  - `api/` - Desktop IPC contract types, split by domain (barrel at `api/index.ts`; `DesktopAPI` master interface lives here). Type domains that exceed 400 lines may be split into a directory with a barrel `index.ts` (this is the first such case). Deep imports through the directory barrel (e.g., `@shared/types/api/snapshots`) are prohibited — always import via the barrel `@shared/types/api`.
