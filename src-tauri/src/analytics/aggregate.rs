...
pub fn compute_analytics(days: u32) -> Result<AnalyticsResponse, String> {
    ...
    for project in &projects {
        ...
        let mut session_files: Vec<_> = match std::fs::read_dir(&project_dir) {
        ...
        for entry in session_files {
            ...
            let modified_at = entry
                ...
                .unwrap_or(0.0);
            // Go has no portable birthtime; it uses mtime as created (aggregate.go:91).
...
fn build_empty_buckets(
    ...
    match granularity {
        ...
        BucketGranularity::Weekly => {
            ...
            while monday <= today {
                // Go uses monday.ISOWeek() → ISO year, not the Gregorian year.
