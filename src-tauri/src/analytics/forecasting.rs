...
pub fn linear_fit(costs: &[f64]) -> (f64, f64) {
    ...
    // Go uses a 1e-300 near-zero guard (forecasting.go:42), not f64::EPSILON.
    if denom.abs() < 1e-300 {
...
pub fn compute_cost_forecast(window_days: u32) -> Result<CostForecast, String> {
    let days = window_days.clamp(2, 90);
    let response = compute_analytics(days)?;
