...
pub struct AnalyticsResponse {
    ...
    // Go's ToolSummary is interface{} + omitempty, always nil (aggregate.go). Tool
    // analytics is Cycle C, not recovered here — kept as a nullable JSON value that
    // is always None, so it is always omitted (byte-parity with Go's nil field).
