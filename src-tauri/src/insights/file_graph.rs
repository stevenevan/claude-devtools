...
fn extract_path(tool_name: &str, input: &serde_json::Value) -> Option<String> {
    ...
    // Empty strings are rejected (fall through to the next candidate), matching Go.
...
/// Runs the file graph scan on a single session JSONL file. Extracted so tests
/// can drive it without a real ~/.claude layout. Mirrors Go's `ScanFile`.
fn scan_file(path: &Path) -> Result<FileGraphResponse, String> {
    ...
    for line in reader.lines().map_while(Result::ok) {
        ...
        let mut paths: Vec<String> = turn_paths.into_iter().collect();
        paths.sort();
    ...
    let mut node_out: Vec<FileGraphNode> = nodes
        .into_iter()
        .map(|(path, a)| {
        ...
        .collect();
    // Deterministic node order by path (matches Go's sort.Slice on Path).
    node_out.sort_by(|a, b| a.path.cmp(&b.path));

    let mut edge_out: Vec<FileGraphEdge> = edges
        .into_iter()
        ...
        .collect();
    // Deterministic edge order by (from, to, kind) (matches Go).
    edge_out.sort_by(|a, b| {
        a.from
            .cmp(&b.from)
...
/// Resolves `<canonical_root>/<baseID>/<sessionID>.jsonl` (baseID = project_id
/// truncated at "::" if present) and runs the scan. Mirrors Go's
/// `ComputeFileGraph`; `canonical_root` is the `~/.claude/projects` directory.
...
mod tests {
    ...
    fn read_edit_write_creates_two_self_edges() {
        ...
        let graph = scan_file(&path).unwrap();
