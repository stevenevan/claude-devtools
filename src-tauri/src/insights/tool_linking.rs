...
fn truncate(text: &str, max_len: usize) -> String {
    if text.len() <= max_len {
        ...
        // Go byte-slices freely; Rust panics on a non-char boundary, so back off
        // to the nearest boundary <= max_len (identical to Go for ASCII).
        let mut end = max_len;
...
/// A `BTreeMap` so keys serialize in sorted order (matches Go callers that sort
/// the map keys before emitting JSON).
pub fn link_tool_calls_to_results(
    steps: &[SemanticStep],
    responses: Option<&[ParsedMessageInput]>,
) -> BTreeMap<String, LinkedToolItem> {
    let mut linked_tools = BTreeMap::new();
